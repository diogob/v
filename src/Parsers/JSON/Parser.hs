...
happyExpList = Happy_Data_Array.listArray (0,17) ([672,32,16,0,0,1,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseJSON","Value","identifier","string","integer","'{'","'}'","'['","']'","','","':'","%eof"]
...
happyReduction_3 (HappyTerminal happy_var_1)
	 =  HappyAbsSyn4
		 (unTok happy_var_1 (\range (L.String string) -> String string)
	)
...
happyNewToken action sts stk
	...
	case tk of {
	L.RangedToken L.EOF _ -> action 14 14 tk (HappyState action) sts stk;
	L.RangedToken (L.Identifier _) _ -> cont 5;
	L.RangedToken (L.String _) _ -> cont 6;
	L.RangedToken (L.Integer _) _ -> cont 7;
	L.RangedToken L.LCurly _ -> cont 8;
	L.RangedToken L.RCurly _ -> cont 9;
	L.RangedToken L.LSquare _ -> cont 10;
	L.RangedToken L.RSquare _ -> cont 11;
	L.RangedToken L.Comma _ -> cont 12;
	L.RangedToken L.Colon _ -> cont 13;
	_ -> happyError' (tk, [])
	})

happyError_ explist 14 tk = happyError' (tk, explist)
